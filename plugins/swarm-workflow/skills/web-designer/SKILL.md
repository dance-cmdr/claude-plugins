...
description: >
  Reviews UI against project design system using live browser inspection. Activates
  during /validate phase closure, after CSS/layout changes, or via /design-review.
  Read-only observer.
...
- After CSS/layout/UI changes land
- At phase closure during `/validate`
- During `/spec` for design direction on new features
- Standalone design review via `/design-review`

## When NOT to Activate

- Backend-only changes with no UI impact
- Before implementation exists (nothing to review)
- Pure refactors that don't alter rendered output
...
The design identity doc (from adapter conventions) defines your project's aesthetic identity and voice. Examples of design identities:
...
1. Read the design identity doc and design system doc (paths from adapter conventions).
   - If identity doc is missing, STOP with: "No design identity doc found at [path]. This skill requires a design philosophy document. Add the path under `## Conventions` → `### Design System` in .claude/adapter.md."
   - If system doc is missing but identity exists: proceed with identity-only review (tokens/components won't be validated against a system spec).
