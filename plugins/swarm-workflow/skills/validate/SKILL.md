...
name: validate
description: >
  Runs full test matrix, regression check, cleanup, code review, security audit,
  documentation update, and phase handoff. Use after /swarm completes, or with
  "validate", "close phase", "final check".
...
Run the security audit from `references/security-audit.md` against all files changed in this phase.

- If Critical or High findings: fix before proceeding
- If Medium/Low only: log findings, continue

Skip if changes are purely documentation, configuration, or styling with no server-side logic.

### 6. Design Review (if frontend changes)

If this phase touched CSS, layout, or UI components, and the web-designer skill is installed:

1. Spawn a design review agent using the web-designer skill (`${CLAUDE_PLUGIN_ROOT}/skills/web-designer/SKILL.md`)
...
### 9. Phase Closure Document (if closing a phase)
...
- [ ] Security audit: no Critical/High findings (or N/A)
